...
const int SH_DEG = 3;
const int NR_TOT_SH_COEFFS = (SH_DEG + 1) * (SH_DEG + 1); // 16
...
float eval_sh(
    ...
    // deg 3
    result += C3[0] * y * (3.0 * xx - yy) * sh3Coeffs[0 * 4 + channel];
    result += C3[1] * xy * z * sh3Coeffs[1 * 4 + channel];
    result += C3[2] * y * (4.0 * zz - xx - yy) * sh3Coeffs[2 * 4 + channel];
    result += C3[3] * z  * (2.0 * zz - 3.0 * xx - 3.0 * yy) * sh3Coeffs[3 * 4 + channel];
    result += C3[4] * x * (4.0 * zz - xx - yy) * sh3Coeffs[4 * 4 + channel];
    result += C3[5] * z * (xx - yy) * sh3Coeffs[5 * 4 + channel];
    result += C3[6] * x * (xx - 3.0 * yy) * sh3Coeffs[6 * 4 + channel];
...
void main() {
    ...
    for (int i = 0; i < NR_COEFFS[0]; i++) {
        ...
        if (visualize_sh_coeffs) {
            gl_FragColor = vec4(sh_0_coeffs_batch.r, sh_0_coeffs_batch.g, sh_0_coeffs_batch.b, 1.0);
            return;
        }
    ...
    for (int i = 0; i < NR_COEFFS[3]; i++) {
        // read coeffs batch
        vec4 sh_3_coeffs_batch = texture(sh_3_coeffs_texture_3D, vec3(vUv, float(i)));
        sh_3_coeffs_batch = sh_3_coeffs_batch * (v_max - v_min) + v_min;
        for (int j = 0; j < 4; j++) {
            sh_coeffs_deg_3[i * 4 + j] = sh_3_coeffs_batch[j];
        }
        // r0, g0, b0, a0, ..., r6, g6, b6, a6
    }
