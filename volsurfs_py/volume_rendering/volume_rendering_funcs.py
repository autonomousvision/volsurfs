...
#             bg_transmittance,
#             weight_per_sample,
#         ) = VolumeRendering.volume_render_nerf(
#             ray_samples_packed,
#             rgb_samples,
#             radiance_samples,
#             ray_t_exit,
...
#         (
#             grad_rgb_samples,
#             grad_radiance_samples,
#         ) = VolumeRendering.volume_render_nerf_backward(
#             grad_pred_rgb,
#             grad_bg_transmittance,
#             grad_weight_per_sample,
#             pred_rgb,
#             ray_samples_packed,
#             rgb_samples,
#             radiance_samples,
...
class CumprodOneMinusAlphaToTransmittanceFunc(Function):
    @staticmethod
    def forward(ctx, ray_samples_packed, alpha):
        transmittance, bg_transmittance = (
            VolumeRendering.cumprod_one_minus_alpha_to_transmittance(
    ...
    def backward(ctx, grad_transmittance, grad_bg_transmittance):
        ...
        # tensor_LV=-tensor_LV
        cumsumLV = VolumeRendering.cumsum_over_rays(
            ray_samples_packed, tensor_LV, True
        )  # the True is for inverse cumsum
        ...
        grad_alpha = None
        grad_alpha = VolumeRendering.cumprod_one_minus_alpha_to_transmittance_backward(
            grad_transmittance,
            grad_bg_transmittance,
            ray_samples_packed,
            alpha,
        ...
        ctx.ray_samples_packed = (
            None  # Release memory in case it's not automatically released
        )

        return None, grad_alpha


class IntegrateWithWeights1DFunc(Function):
    @staticmethod
    def forward(ctx, ray_samples_packed, rgb_samples, weights_samples):
        pred_rgb = VolumeRendering.integrate_with_weights_1d(
            ray_samples_packed, rgb_samples, weights_samples
        )
    ...
    def backward(ctx, grad_pred_rgb):
        ...
        (
            grad_rgb_samples,
            grad_weights_samples,
        ) = VolumeRendering.integrate_with_weights_1d_backward(
            grad_pred_rgb, ray_samples_packed, rgb_samples, weights_samples, pred_rgb
        )
...
class IntegrateWithWeights3DFunc(Function):
    @staticmethod
    def forward(ctx, ray_samples_packed, rgb_samples, weights_samples):
        pred_rgb = VolumeRendering.integrate_with_weights_3d(
            ray_samples_packed, rgb_samples, weights_samples
        )
    ...
    def backward(ctx, grad_pred_rgb):
        ...
        (
            grad_rgb_samples,
            grad_weights_samples,
        ) = VolumeRendering.integrate_with_weights_3d_backward(
            grad_pred_rgb, ray_samples_packed, rgb_samples, weights_samples, pred_rgb
        )
...
class SumOverRayFunc(Function):
    ...
    def forward(ctx, ray_samples_packed, sample_values):
        values_sum_per_ray, values_sum_per_sample = VolumeRendering.sum_over_rays(
    ...
    def backward(ctx, grad_values_sum_per_ray, grad_values_sum_per_sample):
        ...
        grad_sample_values = VolumeRendering.sum_over_rays_backward(
            grad_values_sum_per_ray,
            grad_values_sum_per_sample,
            ray_samples_packed,
            sample_values,
        )
